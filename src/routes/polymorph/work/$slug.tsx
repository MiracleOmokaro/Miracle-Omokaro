...
export const Route = createFileRoute("/polymorph/work/$slug")({
  component: PolyProject,
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [{ title: `${loaderData?.project.title ?? "Project"} — ${POLYMORPH.name}` }],
  }),
});

function PolyProject() {
    ...
    <SiteShell variant="polymorph">
      <article className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
        <p className="text-sm text-subtle">
          <Link to="/polymorph/work" className="hover:text-fg">
            Work
          </Link>
          {" / "}
          {project.client}
        ...
        <div className="mt-8 flex flex-wrap gap-4 text-sm">
          {project.liveUrl ? (
            <a href={project.liveUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-accent">
              Live site <ExternalLink className="size-3.5" />
            </a>
          ) : null}
          {project.githubUrl ? (
            <a href={project.githubUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-muted hover:text-fg">
